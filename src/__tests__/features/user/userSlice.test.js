...
import userReducer, { login, logout, addLoyaltyPoints, redeemLoyaltyPoints } from '../../../features/user/userSlice'

describe('User Slice', () => {
  const initialState = { currentUser: null, loyaltyPoints: 0 }

  describe('login', () => {
    it('should login user with valid email', () => {
      const state = userReducer(initialState, login({
  ...
  describe('redeemLoyaltyPoints', () => {
    it('should redeem points if available', () => {
      const state = { currentUser: null, loyaltyPoints: 100 }
    ...
    it('should not redeem if insufficient points', () => {
      const state = { currentUser: null, loyaltyPoints: 10 }
      const newState = userReducer(state, redeemLoyaltyPoints(50))
